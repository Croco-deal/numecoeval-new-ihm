...
export default function Header() {
    ...
    function handleClick(e) {
      setIsActive({key: e.currentTarget.id, className: 'is-active'})
    }
    return(
      <>
                  ...
                  <div className="collapse navbar-collapse" id="menuPrincipal">
                    ...
                    <nav role="navigation" aria-label="Menu principal">
                      <ul className="navbar-nav nav-lvl--1">
                        <li id="referentiels" onClick={(e) => handleClick(e)} className={`nav-item common-nav-item ${isActive.key==="referentiels" && isActive}`}>
                          <a className="nav-link" href="/referentiels">Référentiels</a>
                        </li>
                        <li id="donnees" onClick={(e) => handleClick(e)} className={`nav-item common-nav-item ${isActive.key==="donnees" && isActive.className}`}>
                            <a className="nav-link" href="/donnees">Données<span className="sr-only">(page courante)</span></a>
                        </li>
                        <li id="dashboard" onClick={(e) => handleClick(e)} className={`nav-item common-nav-item ${isActive.key==="dashboard" && isActive.className}`}>
                            <a className="nav-link" href="/dashboard">Dashboards<span className="sr-only">(page courante)</span></a>                          </li>
                      </ul>
                    </nav>
                    <ul className="nav-right">
                      <li className="nav-right-item collapse-wrapper common-nav-item">
                        <button type="button" className="btn btn--icon-only btn-icon--header" aria-label="Accessibilité" data-toggle="collapse" data-target="#collapseAccess" aria-controls="collapseAccess" aria-expanded="false">
                          <svg className="svg-icon svg-access" aria-hidden="true" focusable="false"><use xlinkHref="svg-icons/icon-sprite.svg#access"></use></svg>
                          <span className="title-responsive">Accessibilité</span>
                        </button>
